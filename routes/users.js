...
//* GET users listing. */
// Because our best path for users.js is "/users" and the route names concatenate, the final path for this route is going to be "localhost:3000/users/all"
router.get('/all', function(req, res, next) {
  res.send("respond with a resource");
});

// THe final url or this rout is goign to be "/users/single"
...
router.post("/create-one", (req, res) => {
  ...
  try {
    ...
    // parse out request data to local variables
        ...
        // In the catch block, we always ant to do 2 things: console.log the error and respond with an error object
        console.log(e);
        res.json({
              success: false,
              error: String(e)
